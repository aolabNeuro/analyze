...
def tabulate_behavior_data_readyset(preproc_dir, subjects, ids, dates, metadata = [], df = None):
    ...
    Returns:
        pd.DataFrame: pandas DataFrame containing the concatenated trial data with columns:
            ...
            | **auditory_start_time (ntrial):** time at which the set of tones starts 
    ...
    new_df['auditory_start_time'] = np.nan
    ...
    for i in range(len(new_df)):
        ...
        # Tones start 
        auditory_on_times = event_times[np.isin(event_codes, task_codes['CUE'])]
        new_df.loc[i, 'auditory start'] = len(auditory_on_times) > 0
        if new_df.loc[i, 'auditory start']:
            new_df.loc[i, 'hold_completed'] = True #if prepbuff state is entered it means hold was completed
            new_df.loc[i, 'auditory_start_time'] = auditory_on_times[0]
        ...
        #slightly longer than lenght of audio file because it is tone onset (not tone offset) 
        if new_df.loc[i, 'auditory_start_time']:
            new_df.loc[i, 'go_cue_time'] = new_df.loc[i,'auditory_start_time'] + new_df.loc[i, 'prepbuff_time'] + new_df.loc[i, 'delay_time']
