...
def get_saccade_label(eye_pos, onset_times, duration, samplerate, target_pos, target_radius, event_times, event_code):
    '''
    ...
    Also returns event codes passed just before a saccade starts and ends. This function is for the standard center out task.
    ...
    for dur, onset in zip(duration, onset_times): 
        ...
        # Get eye positions when a saccade starts and ends
        offset = dur + onset
        onset_pos = eye_pos[int(onset*samplerate),:]
        offset_pos = eye_pos[int(offset*samplerate),:]
        
        # Check if this saccade is for looking at a target or not
        onset_label = (onset_pos[0] - target_pos[:,0])**2 + (onset_pos[1] - target_pos[:,1])**2 <= target_radius**2
        offset_label = (offset_pos[0] - target_pos[:,0])**2 + (offset_pos[1] - target_pos[:,1])**2 <= target_radius**2
    ...
    return onset_target, offset_target, onset_event, offset_event