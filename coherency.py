...
# function[coh, f, S_X, S_Y, coh_err, SX_err, SY_err] = coherency(X, Y, tapers, sampling, fk, pad, pval, flag, contflag)
...
FLAG = 0 may not function as intended
    ...
    if flag == 0:
        ...
        for ch in range(nch):
            ...
            if errorchk: # Estimate error bars using Jacknife
                ...
                lsigX = np.multiply(np.sqrt(K - 1), np.std(jXlsp, 0))
                lsigY = np.multiply(np.sqrt(K - 1), np.std(jYlsp, 0))
                lsigXY = np.multiply(np.sqrt(K - 1), np.std(jcoh, 0))
    ...
    if flag == 1: # Pooling across trials
        ...
        #print(Xk[:,0])
        #print(np.around(np.multiply(Xk,np.conj(Yk)),decimals=4)[:,0])
        ...
        if errorchk: # Estimate error bars using Jacknife
            ...
            lsigX = np.multiply(np.sqrt(nch * K - 1), np.std(jXlsp, 0))
            lsigY = np.multiply(np.sqrt(nch * K - 1), np.std(jYlsp, 0))
            lsigXY = np.multiply(np.sqrt(nch * K - 1), np.std(jcoh, 0))
